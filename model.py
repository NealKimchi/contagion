from mvc import Listener, Listenable
import enum
...
class Individual(Listenable):
    def __init__(self, kind: str, region: 'Population',
                 row: int, col: int):
        ...
        self.row = row
        self.col = col
        # Initially we re 'vulnerable', not yet infected
    ...
    def infect(self):
        ...
        May also be called on 'patient 0' to start sim"""
...
class Population(Listenable):
    ...
    def count_in_state(self, state: Health) -> int:
        """How many individuals are currently in the state?"""
        count = 0
        for row in self.cells:
            for cell in row:
                if cell.state == state:
...
class Typical(Individual):
    """Typical individual. May visit different neightbors
    each day"""
